  ...
  return (
    <footer className="bg-background py-14 sm:py-20">
      <div className="container-wide">
        <div className="grid gap-12 border-b border-emerald-950/10 pb-14 lg:grid-cols-[1.5fr_0.7fr_0.7fr_0.7fr]">
          <div>
            <BrandMark size="sm" />
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Layanan</p>
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Portfolio</p>
            <div className="mt-5 grid gap-3 text-sm text-slate-600">
              <Link href="/portfolio">All Projects</Link>
              <Link href="/portfolio">Case Studies</Link>
              <Link href="/portfolio">Clients</Link>
            </div>
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Contact</p>
            <div className="mt-5 grid gap-3 text-sm text-slate-600">
              <a href={`mailto:${contact.email || "hello@jhiro.id"}`}>{contact.email || "hello@jhiro.id"}</a>
              {contact.phone && <a href={`tel:${contact.phone}`}>{contact.phone}</a>}
              <span>{contact.location || "Jakarta, Indonesia"}</span>
            </div>
          </div>
        </div>
        <div className="flex flex-col gap-4 pt-7 text-xs text-slate-400 sm:flex-row sm:items-center sm:justify-between">
          <p>&copy; {new Date().getFullYear()} {footer.copyright || brandName}</p>
          <div className="flex items-center gap-4">
            ...
            <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-emerald-600 transition-colors" aria-label="LinkedIn">
              <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
            </a>
            <a href="https://twitter.com" target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-emerald-600 transition-colors" aria-label="Twitter">
